...
describe('InvestimentoService', () => {
  ...
  it('getHistoricoInvestimentos should call correct URL', () => {
    ...
    const req = httpMock.expectOne('/api/historico/1');
    expect(req.request.method).toBe('GET');
    req.flush([]);
  });
  ...
  it('simularInvestimento should post payload and return result', () => {
    ...
    service.simularInvestimento(payload).subscribe(res => expect(res.valorFinal).toBe(1100));
    const req = httpMock.expectOne('/api/simular');
    expect(req.request.method).toBe('POST');
  ...
  it('getProdutosRecomendados should call correct URL with perfil', () => {
    ...
    service.getProdutosRecomendados('Conservador').subscribe(res => expect(res.length).toBe(1));
    const req = httpMock.expectOne('/api/produtos/Conservador');
    expect(req.request.method).toBe('GET');
